    ...
    return (
        <header className="max-padd-container w-full z-50">
            <div className="flexBetween py-3">
                ...
                {/* navbar mobile */}
                <div className="">
                    <Navbar containerStyles={`${menuOpened ? "flex items-start flex-col gap-y-8 fixed top-20 right-8 p-12 bg-white rounded-3xl shadow-md w-64 medium-16 ring-1 ring-slate-900/5 transition-all duration-300 z-50" :
                ...
                <div className="flexBetween sm:gap-x-2 bold-16">
                    ...
                    <div className="flexBetween sm:gap-x-6">
                        ...
                        {localStorage.getItem('auth-token') ? <NavLink onClick={() => { localStorage.removeItem('auth-token'); window.location.replace("/") }} to={'logout'} className={"btn-secondary flexCenter gap-x-2 medium-16 rounded-full"}><img src={logout} alt="logutIcon" height={19} width={19} />Logout</NavLink> :
                            <NavLink to={'login'} className={"btn-secondary flexCenter gap-x-2 medium-16 rounded-full"}><img src={user} alt="userIcon" height={19} width={19} />Login</NavLink>}
                    </div>
                </div>
            </div>
        </header>
